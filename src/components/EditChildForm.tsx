...
const EditChildForm = ({ child, onSave, onCancel }: EditChildFormProps) => {
  const [formData, setFormData] = useState<Omit<Child, 'id'>>({
    ...
    activity_level: child.activity_level || 'moderately_active',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanData = {
      ...
      daily_calorie_goal: formData.daily_calorie_goal || undefined,
    };

    onSave(cleanData);
  };

  const handleChange = (field: keyof Omit<Child, 'id'>, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleUseRecommendation = () => {
    ...
    if (recommendations) {
      setFormData(prev => ({
        ...prev,
        daily_calorie_goal: recommendations.calories
      }));
    }
  };
